...
class TransformerDecoderLayer(nn.Layer):
    ...
    def __init__(self,
        ...
        if expert_mode:
            ...
            self.moe_mlp = MoeLayer(num_experts * moe_group_size,
                                    num_experts, 
                                    d_model, 
                                    dim_feedforward, 
                                    top_k,
...
class GPTModel(GPTPretrainedModel):
    ...
    def __init__(self,
        ...
        if topo_scale is not None:
            print(topo_scale)
            self.topo_scale = paddle.slice(topo_scale, axes=[0], starts=[hcg.get_global_rank()], ends=[hcg.get_global_rank()+1])
        else:
