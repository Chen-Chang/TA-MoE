...
def initialize_model_and_expert_group(hcg):
    if hcg.get_pipe_parallel_world_size() == 1:        
        ...
        assert False, "don't support pipeline parallel"
    ...
    for param in model.parameters():
        ...
        if not param.is_distributed:
            paddle.distributed.broadcast(
                param.detach(), src=mp_src_rank, group=mp_group, use_calc_stream=True)

        paddle.distributed.broadcast(
            param.detach(), src=dp_src_rank, group=dp_group, use_calc_stream=True)

def all_reduce_parameters(model, hcg, gate_reduce=True):
    parameters_sync = []
    parameters_sync_for_gate = []
    for param in model.parameters():
        if "expert_" in param.name:
            continue
        if "gate_" in param.name:
            parameters_sync_for_gate.append(param)
        else:
            parameters_sync.append(param)
    if len(parameters_sync_for_gate) > 0 and gate_reduce:
        fused_allreduce_gradients(list(parameters_sync_for_gate), None)
    fused_allreduce_gradients(list(parameters_sync), hcg)

def all_reduce_gate_parameters(model, hcg):
    for param in model.parameters():
        if "gate_" in param.name:
            nranks = paddle.distributed.get_world_size() 
            comm_group = hcg.get_expert_parallel_group()
            div_factor = paddle.to_tensor(nranks, dtype=param.dtype)
            param.stop_gradient=True
            #param.scale(1/div_factor)
            paddle.distributed.all_reduce(param, group=comm_group)
            param.scale_(1/div/factor)
...
def do_train(args):
    ...
    if args.model_name_or_path in pretrained_models_list:
        ...
        if args.topo_file is not None:
            topo_scale = np.loadtxt(args.topo_file)
            model_config['topo_scale'] = paddle.to_tensor(topo_scale)
        else:
            model_config['topo_scale'] = None
        if args.pp_degree == 1:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for f_id in range(num_files):
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                if args.gate != 'naive':
                    aux_loss_list = [l.moe_mlp.gate.get_loss(clear=False)
                        for l in model.gpt.decoder.layers if hasattr(l.moe_mlp, "gate")
                    ]
                    bal_loss = paddle.concat(aux_loss_list).mean()
                    loss += bal_loss * args.balance_loss_weight
                
                loss.backward()
                timers('backward-compute').stop()
                all_reduce_parameters(model, hcg, gate_reduce=(args.gate_reduce_freq==0))
                ...
                if global_step % args.logging_freq == 0:
                    ...
                    logger.info(
                        ...
                        % (global_step, epoch, step, avg_loss, bal_loss, speed, speed *
                ...
                #if args.gate_reduce_freq!=0 and global_step % args.gate_reduce_freq == 0:
                #    with paddle.no_grad():
                #        all_reduce_gate_parameters(model, hcg)
